<div class="mb-4 d-flex justify-content-between">
    ...
    <div>
        <?php if ($pembelian['status_inbound'] != 'Diterima Semua') { ?>
            <?php if ($hasSavedStatus == false) { ?>
                <a href="<?= site_url() ?>warehouse-do_inbound_pembelian/ <?= $pembelian['id'] ?>">
                    <button class="btn btn-sm btn-outline-success">
                        Tambah Inbound <i class="fa-fw fa-solid fa-arrow-turn-down"></i>
                    </button>
                </a>
            <?php } else { ?>
                <button class="btn btn-sm btn-outline-success" onclick="error_tambah_inbound()">
                    Tambah Inbound <i class="fa-fw fa-solid fa-arrow-turn-down"></i>
                </button>
            <?php } ?>
        <?php } ?>
                ...
                <div class="row px-2 mb-0">
                    ...
                    <?php if ($t['status_simpan'] != 'Saved') { ?>
                        <div class="col-md-2 text-end mb-0">
                            <a href="<?= site_url() ?>warehouse-detail_inbound_pembelian/<?= $t['no_inbound'] ?>">
...
<script>
    function show_detail_inbound(id_inbound_pembelian) {
        $.ajax({
            type: 'GET',
            url: '<?= site_url() ?>warehouse-show_detail_inbound/' + id_inbound_pembelian,
            dataType: 'json',
    ...
    function error_tambah_inbound() {
        Swal.fire(
            'Oopss!',
            'masih ada Inbound yang belum disimpan, silahkan dilanjutkan dan disimpan dulu!',
            'error'
        )
    }
</script>