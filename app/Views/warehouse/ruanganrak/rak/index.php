...
<main class="p-md-3 p-2">
    ...
    <div class="d-flex mb-0">
        <div class="me-auto mb-1">
            <h3 style="color: #566573;">Data Rak <?= $nama_gudang ?></h3>
        </div>
        <div class="me-2 mb-1">
            <a class="btn btn-sm btn-outline-dark" href="<?= site_url() ?>warehouse-produkrak/<?= $id_gudang ?>">
                <i class="fa-fw fa-solid fa-arrow-left"></i> Kembali
            </a>
        </div>
        <div class="me-2 mb-1">
            <a class="btn btn-sm btn-outline-primary" href="<?= site_url() ?>warehouse-ruangan/<?= $id_gudang ?>">
                <i class="fa-fw fa-solid fa-arrow-left"></i> Master Ruangan
            </a>
        </div>
        <div class="mb-1">
...
<script>
    ...
    $(document).ready(function() {
        $('#tabel').DataTable({
            processing: true,
            serverSide: true,
            ajax: '<?= site_url() ?>warehouse-getdatarak',
            order: [],
            columns: [{
                    data: 'no',
                    orderable: false,
    ...
    function showModalTambah() {
        $.ajax({
            type: 'GET',
            url: '<?= site_url() ?>warehouse-master_rak/new',
            dataType: 'json',
    ...
    function confirm_delete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $('#form_delete').attr('action', '<?= site_url() ?>warehouse-master_rak/' + id);
                $('#form_delete').submit();
            }
        })
    }
    ...
    function showModalEdit(id) {
        $.ajax({
            type: 'GET',
            url: '<?= site_url() ?>warehouse-master_rak/' + id + '/edit',
            dataType: 'json',
            success: function(res) {
                if (res.data) {
                    $('#isiForm').html(res.data)
