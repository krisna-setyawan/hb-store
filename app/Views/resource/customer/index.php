...
<main class="p-md-3 p-2">
    ...
    <div class="d-flex mb-0">
        ...
        <div class="mb-1">
            <a class="btn btn-sm btn-outline-secondary" href="<?= site_url() ?>resource-customer/new">
                ...
                <?php foreach ($customer as $sp) : ?>
                    <tr>
                        ...
                        <td class="text-center">
                            ...
                            <?php
                            $rule = has_permission('Admin Customer') && (strpos($sp['id_admin'], user()->id) !== false);
                            $in_group = in_groups('Administrator') || in_groups('Owner');
                            if ($rule || $in_group) { ?>
                                <a title="Edit" class="px-2 py-0 btn btn-sm btn-outline-primary" href="<?= site_url() ?>resource-customer/<?= $sp['id'] ?>/edit">
...
<script>
    ...
    function confirm_delete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $('#form_delete').attr('action', '<?= site_url() ?>resource-customer/' + id);
                $('#form_delete').submit();
            }
        })
    }
    ...
    function showModalDetail(id) {
        $.ajax({
            type: 'GET',
            url: '<?= site_url() ?>resource-customer/' + id,
            dataType: 'json',
