...
<script>
    ...
    $(document).ready(function() {
        $('#tabel').DataTable({
            processing: true,
            serverSide: true,
            ajax: '<?= site_url() ?>purchase-getdataproduk',
            order: [],
            columns: [{
                    data: 'no',
                    orderable: false
    ...
    function showModalTambah() {
        $.ajax({
            type: 'GET',
            url: '<?= site_url() ?>purchase-produk/new',
            dataType: 'json',
    ...
    function showModalDetail(id) {
        $.ajax({
            type: 'GET',
            url: '<?= site_url() ?>purchase-produk/' + id,
            dataType: 'json',
            success: function(res) {
                if (res.data) {
                    $('#isiForm').html(res.data)
    ...
    function confirm_delete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $('#form_delete').attr('action', '<?= site_url() ?>purchase-produk/' + id);
                $('#form_delete').submit();
            }
        })
    }
