                ...
                <?php foreach ($karyawan as $karyawan) : ?>
                    <tr>
                        ...
                        <td class="text-center">
                            <a title="List" class="px-2 py-0 btn btn-sm btn-outline-dark" href="<?= site_url() ?>hrm-file-karyawan/<?= $karyawan['id'] ?>">
...
<script>
    ...
    function showModalEdit(id) {
        $.ajax({
            type: 'GET',
            url: '<?= site_url() ?>hrm-karyawan/' + id + '/edit',
            dataType: 'json',
            success: function(res) {
                if (res.data) {
                    $('#isiForm').html(res.data)
    ...
    function confirm_delete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $('#form_delete').attr('action', '<?= site_url() ?>hrm-karyawan/' + id);
                $('#form_delete').submit();
            }
        })
    }
