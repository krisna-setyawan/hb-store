...
<main class="p-md-3 p-2">
    ...
    <div class="d-flex mb-0">
        ...
        <div class="me-2 mb-1">
            <a class="btn btn-sm btn-outline-dark" href="<?= site_url() ?>finance-listakun">
...
<script>
    $(document).ready(function() {
        $('#tabel').DataTable({
            processing: true,
            serverSide: true,
            ajax: '<?= site_url() ?>finance-getDataKategori',
            order: [],
            columns: [{
                    data: 'no',
                    orderable: false
    ...
    function showModalTambah() {
        $.ajax({
            type: 'GET',
            url: '<?= site_url() ?>finance-KategoriAkun/new',
            dataType: 'json',
    ...
    function showModalDetail(id) {
        $.ajax({
            type: 'GET',
            url: '<?= site_url() ?>finance-KategoriAkun/' + id,
            dataType: 'json',
            success: function(res) {
                if (res.data) {
                    $('#isiForm').html(res.data)
    ...
    function showModalEdit(id) {
        $.ajax({
            type: 'GET',
            url: '<?= site_url() ?>finance-KategoriAkun/' + id + '/edit',
            dataType: 'json',
            success: function(res) {
                if (res.data) {
                    $('#isiForm').html(res.data)
